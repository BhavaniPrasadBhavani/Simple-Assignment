...
export default function ChatPanel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Cannot connect to the backend server. Please ensure the backend is running on port 3001.');
      return;
    }
    ...
    try {
      ...
      es.onerror = (error) => {
        ...
        switch (es.readyState) {
          case EventSource.CONNECTING:
            errorMessage += 'Still trying to connect to the server.';
            break;
          case EventSource.CLOSED:
            errorMessage += 'Connection was closed. Please check if the backend is running.';
            break;
          default:
      ...
      updateLastMessage('Failed to start generation. Please try again.');
      setIsGenerating(false);
    }
  };
